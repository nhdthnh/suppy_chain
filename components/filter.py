"""
components/filter.py — Bộ lọc bảng DB.

Chức năng:
  · Tự phát hiện cột barcode / description trong bảng
  · Trả về FilterResult dataclass cho các module khác sử dụng
  · render() → hiển thị UI multiselect chọn bảng
  · get_default_filter_result() → tạo result cứng không cần UI
"""
...
_BARCODE_CANDIDATES = [
    "barcode", "bar_code", "ean", "upc", "code", "ma_vach",
]
_DESC_CANDIDATES = [
    "description", "name", "product_name", "title",
    "ten", "mo_ta", "ten_sp",
]
...
@dataclass
class FilterResult:
    """Kết quả bộ lọc bảng: bảng đã chọn + cột được nhận diện."""

    selected_tables: list[str]      = field(default_factory=list)
    barcode_cols:    dict[str, str] = field(default_factory=dict)
    desc_cols:       dict[str, str] = field(default_factory=dict)
...
def _detect(columns: list[str], candidates: list[str]) -> str | None:
    """Tự động tìm cột phù hợp dựa trên danh sách ứng viên."""
...
def get_default_filter_result(table_name: str) -> FilterResult:
    """Tạo FilterResult cứng cho 1 bảng, không cần giao diện."""
...
def render(key_prefix: str = "filter") -> FilterResult:
    ...
    Returns:
        FilterResult chứa các bảng đã chọn và cột nhận diện.
    """
    tables = list_tables()
    if not tables:
    ...
    with c1:
        st.markdown(
            '<div style="'
            "font-family:'Source Sans',sans-serif;font-weight:500;"
            'font-size:0.82rem;color:#555;margin-top:0.45rem;">'
            "BẢNG</div>",
    ...
    # ── Hiển thị info ─────────────────────────────────────
    with st.expander("ℹ Cột được nhận diện tự động", expanded=False):
        for tbl in selected_tables:
